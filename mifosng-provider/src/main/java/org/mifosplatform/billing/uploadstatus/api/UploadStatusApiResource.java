...
public class UploadStatusApiResource {
	 ...
	 private final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<String>(Arrays.asList("id", "name", "systemDefined","flag"));
	    ...
	    private static final Set<String> UPLOAD_STATUS_PARAMETERS = new HashSet<String>(Arrays.asList("id","uploadProcess","uploadFilePath","processDate",
	            "processRecords","processStatus","errorMessage","flag"));
	 ...
	 @Path("/getData")
	    ...
	    public String retrieveCode( @Context final UriInfo uriInfo) {
		 ...
		 final Set<String> responseParameters = ApiParameterHelper
			...
			final List<UploadStatusData> uploadstatusdata= this.readPlatformService
					.retrieveAllUploadStatusData();
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public Response updateOrder(@PathParam("uploadStatusId") final Long uploadStatusId) {

		//OrdersCommand command =
		// this.apiDataConversionService.convertJsonToOrderCommand(null,null,jsonRequestBody);
		CommandProcessingResult entityIdentifier = this.uploadStatusWritePlatformService.updateUploadStatus(uploadStatusId);
